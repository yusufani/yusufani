...
public class Controller implements Initializable {
   ...
   public void exit() {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        controller=this;
        try {
            SubscribedVehicle subs = new SubscribedVehicle("2134",new Subscription(new Date(5,6,2019),new Date(10,6,2019),"2134"));
            RegularVehicle regular = new RegularVehicle("5666");
            OfficialVehicle polis = new OfficialVehicle("3");
            AutoPark.getInstance().addVehicle(subs);
            AutoPark.getInstance().vehicleEnters(subs.getPlate(),new Time(5,10,Date.getToday()),subs.isSpecial());
            AutoPark.getInstance().vehicleEnters(regular.getPlate(),new Time(10,34,Date.getToday()),regular.isSpecial());
            AutoPark.getInstance().vehicleEnters(polis.getPlate(),new Time(11,13,Date.getToday()),polis.isSpecial());
            init();
        } catch (Exception e) {
            System.out.println("Arac zaten kayitli");
            e.printStackTrace();
        }
    ...
    public void updateHourlyFee(){
        if(TextHourlyFee == null ) System.out.println("null knk ");
