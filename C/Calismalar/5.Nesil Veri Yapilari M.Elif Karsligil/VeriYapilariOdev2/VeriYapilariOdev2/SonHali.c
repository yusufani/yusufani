...
Kullanici* verileriOku(Kullanici*, char *);
void printNext(Kullanici*, char);
...
int main() {
	FILE* p = NULL; // Okunacak dosyanin FIlE Pointeri
	Kullanici static* binaryTree = NULL; // Binary Tree icin root noktasi
	char secim = 1;
	while (secim) {  // Menu sistemi
		...
		case 1: { // Insert New User
			...
			fgets(buffer, 150, p);
			binaryTree = verileriOku(binaryTree, buffer); // InsertNewUser fonksiyonun icinde cagrilmaktadir.
			free(buffer);
			break;
		}
		case 2: {// Delete User
		...
		case 6: { // Print Next
			int TCNO;
			printf("Lutfen agacta kendinden alttaki elemanlarini bulmak istediginiz kullanicinin ID numarasini giriniz:");
			scanf("%d", &TCNO);
...
Kullanici* verileriOku(Kullanici* binaryTree, char * buffer) { // Verilmis input dosyasindan 1 adet kullanici okur ve onu InserNewUser fonksiyonu ile agaca ekler.
	int i;
	char* tmp;
	Kullanici* kullanici = createNewUser(); // bir adet bos kullanici * tipinde eleman uretir.z
	if (strlen(buffer) > 8) { // 8 elemandan dusuk anlamli veri olamaz veya -52 karakteri ile baslayan bir sayi olamaz.
		...
		if (tmp != NULL) // Arkadaslari varsa ekleme yap
		{
			...
			kullanici->friendSize = i - 1; // Friends size'i ayarla
		}
...
void deleteIDFromFriends(Kullanici* node, int tcno) {	//Bu fonksiyon Verilen ID numarasýný diger kullanicilarin friends listesinden siler.
	if (node == NULL) return;
	int i = 0;
...
void bilgileriKopyala(Kullanici* destination, Kullanici* source) {// Source node'undaki bazi bilgileri Destination'a kopyalar.
